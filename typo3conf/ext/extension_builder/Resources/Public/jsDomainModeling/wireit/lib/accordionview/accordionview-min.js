(function() {
	var G = YAHOO.util.Dom,F = YAHOO.util.Event,I = YAHOO.util.Anim;
	var A = function(L, K) {
		L = G.get(L);
		K = K || {};
		if (!L) {
	...
	YAHOO.extend(A, YAHOO.util.Element, {initAttributes:function(K) {
		A.superclass.initAttributes.call(this, K);
		var L = (YAHOO.env.modules.animation) ? true : false;
		this.setAttributeConfig("id", {writeOnce:true,validator:function(M) {
			return(/^[a-zA-Z][\w0-9\-_.:]*$/.test(M))
		},value:G.generateId(),method:function(M) {
			this.get("element").id = M
		}});
		this.setAttributeConfig("width", {value:"400px",method:function(M) {
		...
		this.setAttributeConfig("hoverActivated", {value:false,validator:YAHOO.lang.isBoolean,method:function(M) {
			if (M) {
		...
		for (var M = 0; M < Q.length; M++) {
			if (G.hasClass(Q[M], "nopanel")) {
				N.push({label:"SINGLE_LINK",content:Q[M].innerHTML.replace(/^\s\s*/, "").replace(/\s\s*$/, "")})
			} else {
				if (Q[M].parentNode === O) {
					for (var P = Q[M].firstChild; P && P.nodeType != 1; P = P.nextSibling) {
					}
					if (P) {
						for (var R = P.nextSibling; R && R.nodeType != 1; R = R.nextSibling) {
						}
		...
		G.batch(this._panels, function(L) {
		...
		this.removeChild(G.getElementsByClassName(this.CLASSES.PANEL, this.CONFIG.ITEM_WRAPPER_TAG_NAME, this)[K]);
		var N = [];
		...
		if (T.nodeType === undefined) {
			Q = F.getTarget(T);
			if (!G.hasClass(Q, this.CLASSES.TOGGLE) && !G.hasClass(Q, this.CLASSES.INDICATOR)) {
				return false
			}
		...
		function S(V, X) {
			if (O._ie) {
				var W = G.getElementsByClassName(O.CLASSES.ACCORDION, O.CONFIG.TAG_NAME, V);
				if (W[0]) {
		...
		function P(W, Y) {
			...
			if (this.get("animate")) {
				...
				var X = (this.get("effect")) ? this.get("effect") : YAHOO.util.Easing.easeBoth;
				var V = new I(W, b, d, X);
				if (a) {
		...
		var K = (R.nodeName.toUpperCase() === "SPAN") ? R.parentNode.parentNode : R.parentNode;
		var N = G.getElementsByClassName(this.CLASSES.CONTENT, this.CONFIG.CONTENT_WRAPPER_TAG_NAME, K)[0];
		...
		if (this.get("expandable") !== true) {
			...
			for (var M = 0; M < U; M++) {
				var L = G.hasClass(this._panels[M].firstChild.nextSibling, this.CLASSES.HIDDEN);
				if (!L) {
