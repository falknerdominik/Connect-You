...
(function() {
	...
	C.prototype = {id:null,_el:null,_nodes:null,locked:false,_expandAnim:null,_collapseAnim:null,_animCount:0,maxAnim:2,_hasDblClickSubscriber:false,_dblClickTimer:null,currentFocus:null,singleNodeHighlight:false,_currentlyHighlighted:null,setExpandAnim:function(G) {
		...
		var H = this,L = this._getEventTargetTdEl(J),I,K,G = function() {
			I.toggle();
			I.focus();
			try {
				B.preventDefault(J);
		...
		var K = B.getTarget(J),I = this.getNodeByElement(K),H = I,G = YAHOO.util.KeyListener.KEY;
		switch (J.keyCode) {
			case G.UP:
				do{
...
(function() {
	...
	YAHOO.extend(YAHOO.widget.DateNode, YAHOO.widget.TextNode, {_type:"DateNode",calendarConfig:null,fillEditorContainer:function(G) {
		...
		var I = F.node,H = I.tree.validator,J;
		if (C.isUndefined(D)) {
			J = F.inputElement.value;
		} else {
			var K = F.inputObject,G = K.getSelectedDates()[0],E = [];
...
(function() {
	...
	A.saveEditorValue = function(H) {
		var J = H.node,K = H.inputElement.value,I = J.tree.validator;
		if (F.isFunction(I)) {
			K = I(K, J.label, J);
			if (F.isUndefined(K)) {
				return false;
