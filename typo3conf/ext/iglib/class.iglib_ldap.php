...
class iglib_ldap {
	...
	function connect ($host = null, $port = null, $protocol = null, $charset = null, $type = 0) {
		...
		iglib_ldap::init_charset($charset);
	...
	function get_entries () {
		...
		if ($result) {
			...
			return(iglib_ldap::convert_charset_array($result, $this->ldap_charset, $this->local_charset));
	...
	function get_first_entry () {
		...
		return(iglib_ldap::convert_charset_array(@ldap_get_attributes($this->cid, $this->feid), $this->ldap_charset, $this->local_charset));
	...
	function convert_charset_array($arr, $char1, $char2) {
		...
		while (list($k, $val) = each($arr)) {
			...
			if (is_array($val)) {
				...
				$arr[$k] = iglib_ldap::convert_charset_array($val, $char1, $char2);
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/iglib/class.iglib_ldap.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/iglib/class.iglib_ldap.php']);
}
